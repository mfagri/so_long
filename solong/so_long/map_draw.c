...
/*   Updated: 2022/03/18 19:08:57 by mfagri           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	mouve_player_2(t_data *m)
{
	...
	if (i % 10 == 0)
	{
		...
		{
			j = 0;
			m->img_p_1 = mlx_xpm_file_to_image(m->mlx_ptr, "./img/v1.xpm", &m->a, &m->b);
		}
		else
		{
			j = 1;
			m->img_p_1 = mlx_xpm_file_to_image(m->mlx_ptr, "./img/v2.xpm", &m->a, &m->b);
		}
	}
	if (i == 100000)
		i = 0;
...
void draw_map_2(t_data *data,int j)
{
    if (data->map[data->i][j] == '1')
		mlx_put_image_to_window(data->mlx_ptr, data->mlx_win, data->img_w, j*32, data->i*32);
	if (data->map[data->i][j] == '0')
		mlx_put_image_to_window(data->mlx_ptr, data->mlx_win, data->img_ground, j*32, data->i*32);
	if (data->map[data->i][j] == 'C')
		mlx_put_image_to_window(data->mlx_ptr, data->mlx_win, data->img_c, j*32, data->i*32);
	if (data->map[data->i][j] == 'E')
		mlx_put_image_to_window(data->mlx_ptr, data->mlx_win, data->img_d, j*32, data->i*32);
	if (data->map[data->i][j] == 'P')
		mlx_put_image_to_window(data->mlx_ptr, data->mlx_win, data->img_p_1, j*32, data->i*32);
}
